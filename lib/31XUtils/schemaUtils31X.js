const inputValidation31X = require('./inputValidation31X'),
  ...
  fileUploadTypes = ['application/octet-stream'];
...
module.exports = {
  ...
  parseSpec: function (openApiSpec, options) {
    return schemaUtilsCommon.parseSpec(
      openApiSpec,
      inputValidation31X,
      options
    );
  ...
  fixExamplesByVersion(schema) {
    ...
    const hasExamplesInRoot = schema.hasOwnProperty('examples'),
      hasChildItems =
        schema.hasOwnProperty('type') && schema.hasOwnProperty('items'),
      hasProperties = schema.hasOwnProperty('properties'),
      typeIsAnArray =
        schema.hasOwnProperty('type') && Array.isArray(schema.type);
  ...
  isBinaryContentType(bodyType, contentObj) {
    return (
      Object.keys(contentObj[bodyType]).length === 0 &&
      fileUploadTypes.includes(bodyType)
    );
  },

  getOuterPropsIfIsSupported(schema) {
    const schemaOuterProps = cloneDeep(schema);
    delete schemaOuterProps.$ref;
    return schemaOuterProps;
  },

  addOuterPropsToRefSchemaIfIsSupported(refSchema, outerProps) {
    const resolvedSchema = cloneDeep(refSchema),
      outerKeys = Object.keys(outerProps);
    outerKeys.forEach((key) => {
      resolvedSchema[key] =
        resolvedSchema[key] && Array.isArray(resolvedSchema[key]) ?
