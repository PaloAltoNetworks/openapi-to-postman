const {
  ...
  isTypeValue
} = require('../common/schemaUtilsCommon');
...
function isPmVariable(value) {
  ...
  return _isString(value) && _startsWith(value, '{{') && _endsWith(value, '}}');
}
...
function getAjvValidator(draftToUse) {
  return draftToUse === specialDraft ?
...
function validateSchema(schema, valueToUse, options = {}, jsonSchemaDialect) {
  ...
  filteredValidationError = _filter(
    _get(validate, 'errors', []),
    (validationError) => {
      let dataPath = _get(validationError, 'instancePath', '');
      ...
      if (
        ...
        _has(validationError, 'propertyName')
      ) {
        return false;
      }
      ...
      else if (_includes(IGNORED_KEYWORDS, validationError.keyword)) {
        return false;
      }
      ...
      // Ignore unresolved variables from mismatch if option is set
        ...
        isPmVariable(dataPath === '' ? valueToUse : _get(valueToUse, dataPath))
      ) {
        return false;
      }

      if (
        validationError.keyword === 'type' ||
        validationError.keyword === 'format'
      ) {
        let schemaDataPath = formatDataPath(
            formatSchemaPathFromAJVErrorToConvertToDataPath(
              validationError.schemaPath
            )
          ),
          schemaToUse = schemaDataPath ? _get(schema, schemaDataPath) : schema,
          valueToValidate = dataPath ? _get(valueToUse, dataPath) : valueToUse;
  ...
  filteredValidationError = _sortBy(filteredValidationError, ['dataPath']);
