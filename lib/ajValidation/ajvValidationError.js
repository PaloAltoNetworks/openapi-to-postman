...
function ajvValidationError(ajvValidationErrorObj, data) {
  var mismatchPropName = formatDataPath(
      _get(ajvValidationErrorObj, 'instancePath', '')
    ),
    mismatchObj;
  ...
  mismatchObj = {
    reason:
      `The ${data.humanPropName} property "${mismatchPropName}" ` +
  ...
  switch (ajvValidationErrorObj.keyword) {
    ...
    //   `should have property "${_get(ajvValidationErrorObj, 'params.missingProperty')}" when property ` +
    //   `"${_get(ajvValidationErrorObj, 'params.property')}" is present`;
    //   break;

    case 'required':
      mismatchObj.reasonCode = 'MISSING_IN_REQUEST';
      mismatchObj.reason =
        `The ${data.humanPropName} property "${mismatchPropName}" should have required ` +
        `property "${_get(ajvValidationErrorObj, 'params.missingProperty')}"`;
      break;

    // currently not supported as OAS doesn't support this keyword
    // case 'propertyNames':
    //   mismatchObj.reason = `The ${data.humanPropName} property "${mismatchPropName}" contains invalid ` +
    //     `property named "${_get(ajvValidationErrorObj, 'params.propertyName')}"`;
    //   break;`
