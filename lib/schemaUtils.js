...
const {
    ...
    checkIsCorrectType,
    isKnownType
  } = require('./common/schemaUtilsCommon.js'),
  ...
  _toUpper = require('lodash/toUpper'),
  _cloneDeep = require('lodash/cloneDeep'),
  _forEachRight = require('lodash/forEachRight'),
...
function safeSchemaFaker(
  ...
  var prop,
    ...
    resolvedSchema,
    fakedSchema,
    schemaResolutionCache = _get(schemaCache, 'schemaResolutionCache', {}),
    schemaFakerCache = _get(schemaCache, 'schemaFakerCache', {});
  let concreteUtils =
    components && components.hasOwnProperty('concreteUtils') ?
      components.concreteUtils :
      DEFAULT_SCHEMA_UTILS;

  resolvedSchema = deref.resolveRefs(
    oldSchema,
    parameterSourceOption,
    components,
...
module.exports = {
  ...
  determineOptions: function (analysis, options) {
    ...
    var computedOptions = _clone(options);
  ...
  getCollectionDescription: function (openapi) {
    let description = _get(openapi, 'info.description', '');
    if (_get(openapi, 'info.contact')) {
      let contact = [];
  ...
  getHeaderFamily: function (cTypeHeader) {
    ...
    if (
      ...
      (mediaType.subtype === 'json' || endsWith(mediaType.subtype, '+json'))
    ) {
      return HEADER_TYPE.JSON;
    }
    if (
      (mediaType.type === 'application' || mediaType.type === 'text') &&
      (mediaType.subtype === 'xml' || endsWith(mediaType.subtype, '+xml'))
    ) {
  ...
  assignParameterExamples: function (parameter) {
    let example = _get(parameter, 'example'),
      examples = _values(_get(parameter, 'examples'));

    if (example !== undefined) {
      _set(parameter, 'schema.example', example);
    }
    else if (examples) {
      let exampleToUse = _get(examples, '[0].value');

      !isUndefined(exampleToUse) &&
        _set(parameter, 'schema.example', exampleToUse);
    }
  },
   ...
   */
  convertToPmCollectionVariables: function (
    serverVariables,
    keyName,
    serverUrl = ''
  ) {
    var variables = [];
    if (serverVariables) {
      forOwn(serverVariables, (value, key) => {
  ...
  getRequestParams: function (operationParam, pathParam, components, options) {
    ...
    if (isEmpty(pathParam)) {
      return operationParam;
    }
    else if (isEmpty(operationParam)) {
      return pathParam;
    }
    ...
    for (path in paths) {
      if (paths.hasOwnProperty(path)) {
        ...
        for (i = 0; i < pathLength; i++) {
          ...
          currentNode.children[currentPath[i]].requestCount +=
            currentPathRequestCount;
        ...
        if (currentPathObject.hasOwnProperty('servers')) {
          ...
          collectionVariables[this.fixPathVariableName(path)] =
            pathLevelServers[0];
  ...
  addCollectionItemsFromWebhooks: function (
    spec,
    generatedStore,
    components,
    options,
   ...
   */
  addCollectionItemsUsingPaths: function (
    spec,
    generatedStore,
    components,
    options,
    ...
    */
    ...
    if (folderObj.variables) {
      forOwn(folderObj.variables, (server, key) => {
    ...
    for (child in folderTree.root.children) {
      ...
      if (
        folderTree.root.children.hasOwnProperty(child) &&
        folderTree.root.children[child].requestCount > 0
      ) {
        generatedStore.collection.items.add(
          this.convertChildToItemGroup(
            spec,
            folderTree.root.children[child],
            components,
            options,
            schemaCache,
            variableStore
   ...
   */
  addCollectionItemsUsingTags: function (
    spec,
    generatedStore,
    components,
    options,
    ...
    _forEach(globalTags, (globalTag) => {
      tagFolders[globalTag.name] = {
        description: _get(globalTag, 'description', ''),
        requests: []
      };
    });

    _forEach(paths, (currentPathObject, path) => {
      var commonParams = [],
        collectionVariables,
        pathLevelServers = '';
      ...
      if (currentPathObject.hasOwnProperty('servers')) {
        ...
        _forEach(collectionVariables, (collectionVariable) => {
      ...
      pathMethods = _filter(_keys(currentPathObject), (key) => {
        return _includes(METHODS, key);
      });

      _forEach(pathMethods, (pathMethod) => {
        var summary,
        ...
        if (isEmpty(localTags)) {
          ...
          _forEach(localTags, (localTag) => {
            // add undefined tag object with empty description
            if (!_has(tagFolders, localTag)) {
              tagFolders[localTag] = {
                description: '',
                requests: []
              };
    ...
    _forEachRight(tagFolders, (tagFolder, tagName) => {
      var itemGroup = new sdk.ItemGroup({
        name: tagName,
        description: tagFolder.description
      });

      _forEach(tagFolder.requests, (request) => {
        itemGroup.items.add(
          this.convertRequestToItem(
            spec,
            request,
    ...
    // variableStore contains all the kinds of variable created.
    // Add only the variables with type 'collection' to generatedStore.collection.variables
    _forEach(variableStore, (variable) => {
  ...
  convertPathVariables: function (
    ...
    if (type === 'root' || type === 'method') {
      forOwn(commonPathVars, (value, key) => {
        let description = this.getParameterDescription(value);
        variables.push({
          key: key,
      ...
      _forEach(commonPathVars, (variable) => {
        let fakedData, convertedPathVar;
        ...
        convertedPathVar = this.convertParamsWithStyle(
          variable,
          fakedData,
          PARAMETER_SOURCE.REQUEST,
          components,
        ...
        variables = _concat(variables, convertedPathVar);
      });
    }

    // keep already provided varables (server variables) at last
    return _concat(variables, providedPathVars);
  },
   ...
   * @no-unit-test
   */
  convertChildToItemGroup: function (
    openapi,
    child,
    components,
    options,
    ...
    if (resource.requestCount > 1) {
      ...
      for (
        i = 0, requestCount = resource.requests.length;
        i < requestCount;
        i++
      ) {
        itemGroup.items.add(
      ...
      for (subChild in resource.children) {
        if (
          resource.children.hasOwnProperty(subChild) &&
          resource.children[subChild].requestCount > 0
        ) {
          itemGroup.items.add(
            this.convertChildToItemGroup(
              openapi,
    ...
    for (subChild in resource.children) {
      if (
        resource.children.hasOwnProperty(subChild) &&
        resource.children[subChild].requestCount === 1
      ) {
        return this.convertChildToItemGroup(
          openapi,
          resource.children[subChild],
  ...
  getAuthHelper: function (openapi, securitySet) {
    ...
    if (
      !securitySet ||
      (Array.isArray(securitySet) && securitySet.length === 0)
    ) {
      return null;
    }

    _forEach(securitySet, (security) => {
      if (isObject(security) && isEmpty(security)) {
        helper = {
          type: 'noauth'
        };
        return false;
      }
      securityDef = _get(openapi, ['securityDefs', Object.keys(security)[0]]);
      if (!isObject(securityDef)) {
        return;
      }
      else if (securityDef.type === 'http') {
        if (_toLower(securityDef.scheme) === 'basic') {
          helper = {
            type: 'basic',
            basic: [
        ...
        else if (_toLower(securityDef.scheme) === 'bearer') {
          helper = {
            type: 'bearer',
            bearer: [{ key: 'token', value: '<Bearer Token>' }]
          };
        }
        else if (_toLower(securityDef.scheme) === 'digest') {
          helper = {
            type: 'digest',
            digest: [
          ...
          _toLower(securityDef.scheme) === 'oauth' ||
          _toLower(securityDef.scheme) === 'oauth1'
        ) {
          helper = {
            type: 'oauth1',
            oauth1: [
        ...
        helper = {
          ...
          apikey: [
            {
              key: 'key',
              value: _isString(securityDef.name) ?
                securityDef.name :
                '<API Key Name>'
            },
            ...
            {
              key: 'in',
              value: _includes(['query', 'header'], securityDef.in) ?
                securityDef.in :
                'header'
            }
          ]
  ...
  generateSdkParam: function (param, location) {
    ...
    _has(param, 'disabled') && (generatedParam.disabled = param.disabled);
  ...
  getResponseAuthHelper: function (requestAuthHelper) {
    var responseAuthHelper = {
      ...
      getValueFromHelper = function (authParams, keyName) {
        return _find(authParams, { key: keyName }).value;
      },
      paramLocation,
      description;

    if (!isObject(requestAuthHelper)) {
      return responseAuthHelper;
    }
    description =
      'Added as a part of security scheme: ' + requestAuthHelper.type;
   ...
   */
  convertToPmResponseBody: function (
    contentObj,
    components,
    options,
    schemaCache
    ...
    responseBody = this.convertToPmBodyData(
      contentObj[cTypeHeader],
  ...
  convertToPmBodyData: function (
    ...
    var bodyData = '',
      ...
      schemaType,
      resolveTo = this.resolveToExampleOrSchema(
        requestType,
        options.requestParametersResolution,
    ...
    if (bodyObj.example && (resolveTo === 'example' || !bodyObj.schema)) {
      if (bodyObj.example.hasOwnProperty('$ref')) {
        bodyObj.example = this.getRefObject(
          bodyObj.example.$ref,
          components,
          options
        );
      ...
      !isEmpty(bodyObj.examples) &&
      (resolveTo === 'example' || !bodyObj.schema)
    ) {
      ...
      if (bodyObj.schema.hasOwnProperty('$ref')) {
        let outerProps = concreteUtils.getOuterPropsIfIsSupported(
            bodyObj.schema
          ),
          resolvedSchema;
        ...
        if (outerProps) {
          resolvedSchema = this.getRefObject(
            bodyObj.schema.$ref,
            components,
            options
          );
          bodyObj.schema = concreteUtils.addOuterPropsToRefSchemaIfIsSupported(
            resolvedSchema,
            outerProps
          );
        }
      ...
      if (options.schemaFaker) {
        ...
        if (options.complexityScore === 10) {
          schemaType = _get(
            this.getRefObject(bodyObj.schema.$ref, components, options),
            'type'
          );
   ...
   */
  convertToPmQueryParameters: function (
    param,
    requestType,
    components,
    options,
    ...
    if (param.hasOwnProperty('schema')) {
      ...
      return this.convertParamsWithStyle(
        param,
        paramValue,
        PARAMETER_SOURCE.REQUEST,
        components,
  ...
  extractDeepObjectParams: function (deepObject, objectKey) {
    ...
    _forEach(deepObject, (value, key) => {
      if (typeof value === 'object') {
        extractedParams = _concat(
          extractedParams,
          this.extractDeepObjectParams(value, objectKey + '[' + key + ']')
        );
      }
      else {
   ...
   */
  convertParamsWithStyle: function (
    param,
    paramValue,
    parameterSource,
    components,
    ...
    let {
      ...
      keyValueSeparator,
      isExplodable
    } = this.getParamSerialisationInfo(
      param,
      parameterSource,
      components,
      schemaCache
    ...
    switch (style) {
      case 'form':
        if (explode && isObject(paramValue)) {
          _forEach(paramValue, (value, key) => {
            pmParams.push(
              this.generateSdkParam(
                {
                  key: _isArray(paramValue) ? paramName : key,
        ...
        if (
          explode &&
          _get(param, 'schema.type') === 'object' &&
          isEmpty(_get(param, 'schema.properties'))
        ) {
          return pmParams;
        }
        break;
      case 'deepObject':
        if (isObject(paramValue)) {
          let extractedParams = this.extractDeepObjectParams(
            paramValue,
            paramName
          );

          _forEach(extractedParams, (extractedParam) => {
            pmParams.push(
              this.generateSdkParam(
                {
    ...
    if (isObject(paramValue)) {
      _forEach(paramValue, (value, key) => {
        ...
        !isEmpty(serialisedValue) && (serialisedValue += propSeparator);
  ...
  convertToPmHeader: function (
    ...
    convertedHeader = _get(
      this.convertParamsWithStyle(
        header,
        fakeData,
        parameterSource,
        components,
  ...
  convertToPmBody: function (
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.convertToPmBodyData(
        contentObj[URLENCODED],
        requestType,
        URLENCODED,
        PARAMETER_SOURCE.REQUEST,
        options.indentCharacter,
      ...
      encoding = contentObj[URLENCODED].encoding ?
        contentObj[URLENCODED].encoding :
        {};

      if (
        contentObj[URLENCODED].hasOwnProperty('schema') &&
        contentObj[URLENCODED].schema.hasOwnProperty('$ref')
      ) {
        contentObj[URLENCODED].schema = this.getRefObject(
          contentObj[URLENCODED].schema.$ref,
          components,
          options
        );
      }
      ...
      forOwn(bodyData, (value, key) => {
        if (_get(contentObj[URLENCODED], 'schema.type') === 'object') {
          description = _get(
            contentObj[URLENCODED],
          ...
          required = _includes(
            _get(contentObj[URLENCODED], ['schema', 'required']),
            key
          );
          enumValue = _get(contentObj[URLENCODED], [
            'schema',
            'properties',
            key,
            'enum'
        ...
        isBoolean(required) && (encoding[key].required = required);
        encoding[key].description = description;

        params = this.convertParamsWithStyle(
          encoding[key],
          value,
          PARAMETER_SOURCE.REQUEST,
          components,
      ...
      encoding = contentObj[FORM_DATA].encoding ?
        contentObj[FORM_DATA].encoding :
        {};
      // create the form parameters and add it to the request body object
      forOwn(bodyData, (value, key) => {
        if (_get(contentObj[FORM_DATA], 'schema.type') === 'object') {
          description = _get(
            contentObj[FORM_DATA],
            ['schema', 'properties', key, 'description'],
            ''
          ...
          required = _includes(
            _get(contentObj[FORM_DATA], ['schema', 'required']),
            key
          );
          enumValue = _get(contentObj[FORM_DATA], [
            'schema',
            'properties',
            key,
            'enum'
        ...
        if (encoding.hasOwnProperty(key)) {
          forOwn(encoding[key].headers, (value, key) => {
            if (key !== 'Content-Type') {
              if (encoding[key].headers[key].hasOwnProperty('$ref')) {
                encoding[key].headers[key] = getRefObject(
                  encoding[key].headers[key].$ref,
                  components,
                  options
                );
              }
        ...
        originalParam = _get(contentObj[FORM_DATA], [
          'schema',
          'properties',
          key
        ]);
      ...
      if (concreteUtils.isBinaryContentType(bodyType, contentObj)) {
        updateOptions = {
          mode: 'file'
        };
      }
  ...
  convertToPmResponse: function (
    ...
    forOwn(response.headers, (value, key) => {
      if (_toLower(key) !== 'content-type') {
        if (_get(value, '$ref')) {
        ...
        if (!_includes(IMPLICIT_HEADERS, _toLower(key))) {
    ...
    responseBodyWrapper = this.convertToPmResponseBody(
      response.content,
      components,
      options,
      schemaCache
    ...
    if (responseBodyWrapper.contentTypeHeader) {
      ...
      responseHeaders.push({
        key: 'Content-Type',
        value: responseBodyWrapper.contentTypeHeader
      });
      if (
        this.getHeaderFamily(responseBodyWrapper.contentTypeHeader) ===
        HEADER_TYPE.JSON
      ) {
      ...
      if (
        this.getHeaderFamily(Object.keys(response.content)[0]) ===
        HEADER_TYPE.JSON
      ) {
        previewLanguage = PREVIEW_LANGUAGE.JSON;
      }
    ...
    code = code === 'default' ? 500 : toSafeInteger(code);
  ...
  getRefObject: function ($ref, components, options) {
    ...
    if (savedSchema.length < 3) {
      ...
      return {
        value: `reference ${$ref} not found in the given specification`
      };
    }

    if (savedSchema[0] !== 'components' && savedSchema[0] !== 'paths') {
      console.warn(
        `Error reading ${$ref}. Can only use references from components and paths`
      );
      return {
        value: `Error reading ${$ref}. Can only use references from components and paths`
      };
    }
    ...
    refObj = _get(components, savedSchema);
  ...
  getFixedPartsFromPathSegment: function (segment, pathType = 'collection') {
    var tempSegment = segment,
      ...
      varMatches = segment.match(
        pathType === 'schema' ? /(\{[^\/\{\}]+\})/g : /(\{\{[^\/\{\}]+\}\})/g
      ),
      fixedParts = [];

    _forEach(varMatches, (match) => {
  ...
  cleanWebhookName: function (webhookName) {
    return webhookName.replace(/[:/]/g, '_').replace(/[{}]/g, '');
  },
  ...
  convertRequestToItem: function (
    ...
    var reqName,
      ...
      authMeta,
      swagResponse,
      localServers = fromWebhooks ?
        '' :
        _get(operationItem, 'properties.servers'),
      exampleRequestBody,
      sanitizeResult,
      globalServers = fromWebhooks ? '' : _get(operationItem, 'servers'),
      responseMediaTypes,
      acceptHeader;
    ...
    sanitizeResult.collectionVars.forEach((element) => {
      if (!variableStore[element.name]) {
        ...
        variableStore[element.name] = _assign(convertedPathVar, {
          key: element.name,
          type: 'collection'
        });
      }
    ...
    if (Array.isArray(localServers) && localServers.length) {
      ...
      if (serverObj.variables) {
        forOwn(serverObj.variables, (value, key) => {
          pathVarArray.push({
            name: key,
            value: value.default || '',
        ...
        sanitizeResult.pathVars.forEach((element) => {
          serverObj.pathVariables[element.name] =
            serverObj.variables[element.name];
        });

        // use this new filterd serverObj.pathVariables
        // to generate pm path variables.
        pathVarArray = this.convertPathVariables(
          'method',
          [],
          serverObj.pathVariables,
          components,
      ...
      if (Array.isArray(globalServers) && globalServers.length) {
        ...
        displayUrl =
          '{{' + this.fixPathVariableName(operationItem.path) + '}}' + reqUrl;
      }
    ...
    if (authMeta && authMeta.currentHelper && authMap[authMeta.currentHelper]) {
      ...
      thisAuthObject[authMap[authMeta.currentHelper]] =
        authMeta.helperAttributes;
    ...
    _forEach(reqParams.query, (queryParam) => {
      this.convertToPmQueryParameters(
        queryParam,
        REQUEST_TYPE.ROOT,
        components,
        options,
    ...
    item.request.url.variables.assimilate(
      this.convertPathVariables(
        'param',
        pathVarArray,
        reqParams.path,
        components,
    ...
    if (
      item.request.url.variables.members &&
      item.request.url.variables.members.length > 0
    ) {
      item.request.url.variables.members = _map(
        item.request.url.variables.members,
        (m) => {
    ...
    _forEach(reqParams.header, (header) => {
      if (
        options.keepImplicitHeaders ||
        !_includes(IMPLICIT_HEADERS, _toLower(_get(header, 'name')))
      ) {
    ...
    if (reqBody) {
      ...
      if (
        !options.keepImplicitHeaders ||
        !_find(reqParams.header, (h) => {
          return _toLower(_get(h, 'name')) === 'content-type';
        })
      ) {
    ...
    if (operation.responses) {
      ...
      if (options.includeAuthInfoInExample) {
        ...
        if (isEmpty(authHelper)) {
          responseAuthHelper = this.getResponseAuthHelper(
            this.getAuthHelper(openapi, openapi.security)
          );
        }

        authQueryParams = _map(responseAuthHelper.query, (queryParam) => {
          // key-value pair will be added as transformed query string
          return queryParam.key + '=' + queryParam.value;
        });
      }

      forOwn(operation.responses, (response, code) => {
        let originalRequestHeaders = [],
          originalRequestQueryParams = this.convertToPmQueryArray(
            reqParams,
            REQUEST_TYPE.EXAMPLE,
            components,
            options,
        ...
        // setting URL
        const clonedItemURL = _cloneDeep(item.request.url);
        ...
        try {
          exampleRequestBody = this.convertToPmBody(
            operationItem.properties.requestBody,
            REQUEST_TYPE.EXAMPLE,
            components,
            options,
            schemaCache
          );
          thisOriginalRequest.body = exampleRequestBody.body ?
            exampleRequestBody.body.toJSON() :
            {};
        }
        catch (e) {
        ...
        if (isEmpty(acceptHeader)) {
          responseMediaTypes = _keys(_get(swagResponse, 'content'));
        ...
        convertedResponse = this.convertToPmResponse(
          swagResponse,
          code,
          thisOriginalRequest,
          components,
    ...
    if (!isEmpty(acceptHeader) && !item.request.headers.has('accept')) {
   ...
   */
  convertToPmQueryArray: function (
    reqParams,
    requestType,
    components,
    options,
  ...
  findMatchingRequestFromSchema: function (method, url, schema, options) {
    ...
    try {
      ...
      console.warn(
        'Error decoding request URI endpoint. URI: ',
        url,
        'Error',
        e
      );
    ...
    forOwn(schemaPathItems, (pathItemObject, path) => {
      if (!pathItemObject) {
      ...
      pathItemObject.parameters = _reduce(
      ...
      let schemaMatchResult = this.getPostmanUrlSchemaMatchScore(
        pathToMatch,
        path,
        options
      );
    ...
    filteredPathItemsArray = orderBy(
      filteredPathItemsArray,
      ['fixedMatchedSegments', 'variableMatchedSegments'],
      ['desc', 'desc']
    );

    _each(filteredPathItemsArray, (fp) => {
      let path = fp.path,
      ...
      // filter empty parameters
      matchedPath.parameters = _reduce(
      ...
      matchedPath.parameters = _map(matchedPath.parameters, (commonParam) => {
        ...
        _map(pathItemObject.parameters || [], (commonParam) => {
      ...
      retVal.push({
        ...
        path: _assign(matchedPath, { schemaPathName: path }),
  ...
  isPmVariable: function (value) {
    ...
    return _isString(value) && startsWith(value, '{{') && endsWith(value, '}}');
  },

  /**
   * This function is little modified version of lodash _get()
  ...
  getPathValue: function (sourceValue, dataPath, fallback) {
    return dataPath === '' ?
      sourceValue :
      _get(sourceValue, dataPath, fallback);
  },
  ...
  getSecurityParams: function (components, location) {
    let securityDefs = _get(components, 'securitySchemes', {}),
      securityParams = [];

    _forEach(securityDefs, (securityDef) => {
      // Currently we only apply header and query for apiKey type of security param during conversion
      if (
        _get(securityDef, 'type') === 'apiKey' &&
        _get(securityDef, 'in') === location
      ) {
   ...
   * }
   */
  getParamSerialisationInfo: function (
    param,
    parameterSource,
    components,
    schemaCache
    ...
    var paramName = _get(param, 'name'),
      paramSchema = deref.resolveRefs(
        _cloneDeep(param.schema),
        parameterSource,
        components,
        _get(schemaCache, 'schemaResolutionCache')
      ),
      style, // style property defined/inferred from schema
      explode, // explode property defined/inferred from schema
    ...
    switch (param.in) {
      case 'path':
        style = _includes(['matrix', 'label', 'simple'], param.style) ?
          param.style :
          'simple';
        break;
      case 'query':
    ...
    explode = isBoolean(param.explode) ?
      param.explode :
      _includes(['form', 'deepObject'], style);
    ...
    switch (style) {
      case 'matrix':
        ...
        startValue =
          ';' +
          (paramSchema.type === 'object' && explode ? '' : paramName + '=');
   ...
   */
  deserialiseParamValue: function (
    param,
    paramValue,
    parameterSource,
    components,
    ...
    // for invalid param object return null
    if (!isObject(param) || !_isString(paramValue)) {
      return null;
    }
    ...
    paramValue = paramValue.slice(
      paramValue.indexOf(startValue) === 0 ? startValue.length : 0
    );
    ...
    if (constructedValue) {
      ...
      _forEach(allProps, (element, index) => {
        let keyValArray;

        if (propSeparator === keyValueSeparator && isExplodable) {
          if (isEvenNumber(index)) {
            keyValArray = _slice(allProps, index, index + 2);
          }
          else {
            return;
          }
        ...
        if (paramSchema.type === 'object') {
          _set(
            constructedValue,
            keyValArray[0],
            convertToDataType(keyValArray[1])
          );
        }
        else if (paramSchema.type === 'array') {
          constructedValue.push(
            convertToDataType(_get(keyValArray, '[1]', element))
          );
        }
      });
    }
  ...
  parseMediaType: function (str) {
    ...
    if (match) {
      ...
      type = _toLower(match[1]);
      subtype = _toLower(match[2]);
    }
  ...
  extractChildParamSchema: function (schema, paramKey, metaInfo) {
    ...
    _forEach(_get(schema, 'properties', {}), (value, key) => {
      if (_get(value, 'type') === 'object') {
        childParamSchemas = _concat(
          childParamSchemas,
          this.extractChildParamSchema(value, `${paramKey}[${key}]`, metaInfo)
        );
      }
      else {
        let required = _get(metaInfo, 'required') || false,
          description = _get(metaInfo, 'description') || '',
          pathPrefix = _get(metaInfo, 'pathPrefix');
  ...
  getJsonContentType: function (contentObj) {
    let jsonContentType = _find(_keys(contentObj), (contentType) => {
  ...
  checkValueAgainstSchema: function (
    ...
    if (compositeSchema) {
      ...
      async.map(
        ...
        (elementSchema, cb) => {
          setTimeout(() => {
            this.checkValueAgainstSchema(
              ...
              }[${_findIndex(compositeSchema, elementSchema)}]`,
              elementSchema,
              parameterSourceOption,
              components,
              options,
        ...
        (err, results) => {
          ...
          sortedResults = sortBy(results, (res) => {
            return res.length;
          });
    ...
    // When processing a reference, schema.type could also be undefined
      ...
      if (isKnownType(schema)) {
        ...
        if (
          options.ignoreUnresolvedVariables &&
          this.isPmVariable(valueToUse)
        ) {
          isCorrectType = true;
        }
        else {
        ...
        if (!isCorrectType) {
          ...
          if (
            _includes(['QUERYPARAM', 'PATHVARIABLE', 'HEADER'], property) &&
          ...
          if (options.suggestAvailableFixes) {
            mismatchObj.suggestedFix = {
              ...
              suggestedValue: safeSchemaFaker(
                openApiSchemaObj || {},
                'example',
                PROCESSING_TYPE.VALIDATION,
                parameterSourceOption,
        ...
        if (isCorrectType && needJsonMatching) {
          ...
          if (!isEmpty(filteredValidationError)) {
            let mismatchObj,
              suggestedValue,
              fakedValue = safeSchemaFaker(
                openApiSchemaObj || {},
                'example',
                PROCESSING_TYPE.VALIDATION,
                parameterSourceOption,
            ...
            if (options.detailedBlobValidation && needJsonMatching) {
              _forEach(filteredValidationError, (ajvError) => {
                let localSchemaPath = ajvError.schemaPath
                    .replace(/\//g, '.')
                    .slice(2),
                ...
                mismatchObj = _assign(
                  {
                    property: property,
                    transactionJsonPath:
                      jsonPathPrefix + formatDataPath(ajvError.instancePath),
                ...
                if (options.suggestAvailableFixes) {
                  mismatchObj.suggestedFix = {
                    key: _split(dataPath, '.').pop(),
                    actualValue: this.getPathValue(valueToUse, dataPath, null),
                    suggestedValue: this.getSuggestedValue(
                      fakedValue,
                      valueToUse,
                      ajvError
                    )
              ...
              if (options.suggestAvailableFixes) {
                suggestedValue = _cloneDeep(valueToUse);
                ...
                _forEach(filteredValidationError, (ajvError) => {
                  ...
                  // for empty string _set creates new key with empty string '', so separate handling
                  if (dataPath === '') {
                    suggestedValue = this.getSuggestedValue(
                      fakedValue,
                      suggestedValue,
  ...
  checkPathVariables: function (
    ...
    pmPathVariables = this.findPathVariablesFromSchemaPath(
      schemaPath.schemaPathName
    );
    schemaPathVariables = _filter(schemaPath.parameters, (param) => {
      // exclude path variables stored as collection variable from being validated further
      return param.in === 'path' && _includes(pmPathVariables, param.name);
    });
    ...
    async.map(
      ...
      (pathVar, cb) => {
        let mismatches = [],
          resolvedParamValue,
          index = _findIndex(determinedPathVariables, pathVar);

        const schemaPathVar = _find(schemaPathVariables, (param) => {
        ...
        resolvedParamValue = this.deserialiseParamValue(
          schemaPathVar,
          pathVar.value,
          PARAMETER_SOURCE.REQUEST,
          components,
      ...
      (err, res) => {
        ...
        _each(schemaPathVariables, (pathVar) => {
          if (
            !_find(determinedPathVariables, (param) => {
              // only consider variable matching if url path variables is not allowed
              return (
                param.key === pathVar.name &&
                (options.allowUrlPathVarMatching || param._varMatched)
              );
            })
          ) {
            // assign parameter example(s) as schema examples;
        ...
        return callback(null, _concat(_flatten(res), mismatches));
      }
    );
  },
  ...
  checkMetadata(
    ...
    let expectedReqName,
      expectedReqDesc,
      reqNameMismatch,
      actualReqName = _get(transaction, 'name'),
      trimmedReqName,
      actualReqDesc,
      mismatches = [],
      mismatchObj,
    ...
    // handling path templating in request url if any
    // convert all {anything} to {{anything}}
    reqUrl = this.fixPathVariablesInUrl(
      pathRoute.slice(pathRoute.indexOf('/'))
    );
    ...
    actualReqDesc = isObject(_get(transaction, 'request.description')) ?
      _get(transaction, 'request.description.content') :
      _get(transaction, 'request.description');
    ...
    switch (options.requestNameSource) {
      case 'fallback': {
        ...
        expectedReqName =
          schemaPath.summary ||
          utils.insertSpacesInName(schemaPath.operationId) ||
          reqUrl;
      ...
      case 'url': {
        ...
        reqNameMismatch = !endsWith(actualReqName, reqUrl);
        break;
      }
      default: {
     ...
     * are not empty. isEmpty() returns true for null/undefined/''(empty string)
     * i.e. collection desc = null and schema desc = '', for this case no mismatch will occurr
     */
    if (
      (!isEmpty(actualReqDesc) || !isEmpty(expectedReqDesc)) &&
      actualReqDesc !== expectedReqDesc
    ) {
      mismatchObj = {
  ...
  checkQueryParams(
    ...
    let parsedUrl = require('url').parse(requestUrl),
      schemaParams = _filter(schemaPath.parameters, (param) => {
    ...
    securityParams = _map(
      this.getSecurityParams(_get(components, 'components'), 'query'),
      'name'
    );
    requestQueryParams = _filter(requestQueryParams, (pQuery) => {
      return !_includes(securityParams, pQuery.key);
    });

    // resolve schema params
    // below will make sure for exploded params actual schema of property present in collection is present
    _forEach(schemaParams, (param) => {
      let pathPrefix = param.pathPrefix,
        ...
        isPropSeparable = _includes(['form', 'deepObject'], style);
      ...
      if (isPropSeparable && paramSchema.type === 'array' && explode) {
         ...
         */
        if (!_includes(['array', 'object'], _get(paramSchema, 'items.type'))) {
          // add schema of corresponding items instead array
          resolvedSchemaParams.push(
            _assign({}, param, {
              schema: _get(paramSchema, 'items'),
              isResolvedParam: true
            })
          );
        }
        ...
        if (style === 'deepObject') {
          resolvedSchemaParams = _concat(
            resolvedSchemaParams,
            this.extractChildParamSchema(paramSchema, param.name, {
              required: _get(param, 'required'),
              pathPrefix,
              description: _get(param, 'description')
            })
          );
        }
        else {
          ...
          _forEach(
            _get(paramSchema, 'properties', {}),
            (propSchema, propName) => {
              resolvedSchemaParams.push({
                name: propName,
                schema: propSchema,
                required: _get(param, 'required') || false,
                description: _get(param, 'description'),
                isResolvedParam: true,
                pathPrefix
              });
    ...
    return async.map(
      ...
      (pQuery, cb) => {
        let mismatches = [],
          index = _findIndex(requestQueryParams, pQuery),
          resolvedParamValue = pQuery.value;

        const schemaParam = _find(resolvedSchemaParams, (param) => {
          return param.name === pQuery.key;
        });
      ...
      (err, res) => {
        ...
        _each(
          _filter(resolvedSchemaParams, (q) => {
            return q.required;
          }),
          (qp) => {
            if (
              !_find(requestQueryParams, (param) => {
   ...
   */
  checkContentTypeHeader: function (
    headers,
    transactionPathPrefix,
    schemaPathPrefix,
    contentObj,
    ...
    let mediaTypes = [],
      ...
      hasComputedType,
      humanPropName =
        mismatchProperty === 'HEADER' ? 'header' : 'response header',
      mismatches = [];

    // get all media types present in content object
    _forEach(_keys(contentObj), (contentType) => {
    ...
    _forEach(headers, (header, index) => {
      if (_toLower(header.key) === 'content-type') {
    ...
    if (!isEmpty(contentHeader) && isEmpty(mediaTypes)) {
      // ignore mismatch for default header (text/plain) added by conversion
      if (
        options.showMissingInSchemaErrors &&
        _toLower(contentHeaderMediaType) !== TEXT_PLAIN
      ) {
        mismatches.push({
          property: mismatchProperty,
          transactionJsonPath:
            transactionPathPrefix + `[${contentHeaderIndex}]`,
          schemaJsonPath: null,
          reasonCode: 'MISSING_IN_SCHEMA',
          // Reason for missing in schema suggests that certain media type in req/res body is not present
          reason:
            `The ${
              mismatchProperty === 'HEADER' ? 'request' : 'response'
              // eslint-disable-next-line no-useless-concat
            } body should have media type` + ` "${contentHeaderMediaType}"`
        });
      }
    }

    // No request/response content-type header
    else if (isEmpty(contentHeader) && !isEmpty(mediaTypes)) {
      let mismatchObj = {
        property: mismatchProperty,
        transactionJsonPath: transactionPathPrefix,
        schemaJsonPath: schemaPathPrefix,
    ...
    // Invalid type of header found
    else if (!isEmpty(contentHeader)) {
      let mismatchObj,
        matched = false;

      // wildcard header matching
      _forEach(mediaTypes, (mediaType) => {
        let transactionHeader = _split(contentHeaderMediaType, '/'),
          headerTypeMatched =
            mediaType.type === '*' || mediaType.type === transactionHeader[0],
          headerSubtypeMatched =
            mediaType.subtype === '*' ||
            mediaType.subtype === transactionHeader[1];
      ...
      if (!matched) {
        mismatchObj = {
          property: mismatchProperty,
          transactionJsonPath:
            transactionPathPrefix + `[${contentHeaderIndex}].value`,
          schemaJsonPath: schemaPathPrefix,
          reasonCode: 'INVALID_TYPE',
          reason:
            `The ${humanPropName} "Content-Type" needs to be "${suggestedContentHeader}",` +
  ...
  checkRequestHeaders: function (
    ...
    let schemaHeaders = _filter(schemaPath.parameters, (param) => {
        return param.in === 'header';
      }),
      // key name of headers which are added by security schemes
      securityHeaders = _map(
        this.getSecurityParams(_get(components, 'components'), 'header'),
        'name'
      ),
      // filter out headers for following cases
      reqHeaders = _filter(headers, (header) => {
        // 1. which need explicit handling according to schema (other than parameters object)
        // 2. which are added by security schemes
        return (
          !_includes(IMPLICIT_HEADERS, _toLower(_get(header, 'key'))) &&
          !_includes(securityHeaders, header.key)
        );
      }),
    ...
    return async.map(
      ...
      (pHeader, cb) => {
        let mismatches = [],
          resolvedParamValue,
          index = _findIndex(headers, pHeader); // find actual index from collection request headers

        const schemaHeader = _find(schemaHeaders, (header) => {
          return header.name === pHeader.key;
        });
      ...
      (err, res) => {
        let mismatches = [],
          mismatchObj,
          reqBody = _get(schemaPath, 'requestBody'),
        ...
        if (reqBody) {
          ...
          contentHeaderMismatches = this.checkContentTypeHeader(
            headers,
            transactionPathPrefix,
            schemaPathPrefix + '.requestBody.content',
            _get(reqBody, 'content'),
            mismatchProperty,
            options
          );
        }
        _each(
          _filter(schemaHeaders, (h) => {
            // exclude non-required and implicit header from further validation
            return h.required && !_includes(IMPLICIT_HEADERS, _toLower(h.name));
          }),
          (header) => {
            if (
              !_find(reqHeaders, (param) => {
                return param.key === header.name;
              })
            ) {
              // assign parameter example(s) as schema examples;
              this.assignParameterExamples(header);
              ...
              mismatchObj = {
                ...
                transactionJsonPath: transactionPathPrefix,
                schemaJsonPath:
                  header.pathPrefix + '[?(@.name==\'' + header.name + '\')]',
                reasonCode: 'MISSING_IN_REQUEST',
        ...
        return callback(
          null,
          _concat(contentHeaderMismatches, _flatten(res), mismatches)
        );
      }
    );
  },

  checkResponseHeaders: function (
    schemaResponse,
    headers,
    transactionPathPrefix,
    schemaPathPrefix,
    ...
    let schemaHeaders,
      ...
      resHeaders = _filter(headers, (header) => {
        return !_includes(IMPLICIT_HEADERS, _toLower(_get(header, 'key')));
      }),
    ...
    return async.map(
      ...
      (pHeader, cb) => {
        let mismatches = [],
          index = _findIndex(headers, pHeader); // find actual index from collection response headers

        const schemaHeader = _get(schemaHeaders, pHeader.key);
        ...
        setTimeout(() => {
          ...
          return this.checkValueAgainstSchema(
            mismatchProperty,
            transactionPathPrefix + `[${index}].value`,
            pHeader.key,
            pHeader.value,
      ...
      (err, res) => {
        let mismatches = [],
          ...
          contentHeaderMismatches = this.checkContentTypeHeader(
            headers,
            transactionPathPrefix,
            schemaPathPrefix + '.content',
            _get(schemaResponse, 'content'),
            mismatchProperty,
            options
          );

        _each(
          _filter(schemaHeaders, (h, hName) => {
            ...
            h.name = hName;
            // exclude non-required and implicit header from further validation
            return h.required && !_includes(IMPLICIT_HEADERS, _toLower(hName));
          }),
          (header) => {
            if (
              !_find(resHeaders, (param) => {
                return param.key === header.name;
              })
            ) {
              // assign parameter example(s) as schema examples;
              ...
              mismatchObj = {
                ...
                transactionJsonPath: transactionPathPrefix,
                schemaJsonPath:
                  schemaPathPrefix + '.headers[\'' + header.name + '\']',
                reasonCode: 'MISSING_IN_REQUEST',
  ...
  checkRequestBody: function (
    ...
    if (!isEmpty(_get(schemaPath, 'requestBody.$ref'))) {
      schemaPath.requestBody = this.getRefObject(
        schemaPath.requestBody.$ref,
        components,
        options
      );
    }
    ...
    jsonContentType = this.getJsonContentType(
      _get(schemaPath, 'requestBody.content', {})
    );
    jsonSchemaBody = _get(schemaPath, [
      'requestBody',
      'content',
      jsonContentType,
      'schema'
    ...
    if (requestBody && requestBody.mode === 'raw' && jsonSchemaBody) {
      setTimeout(() => {
        return this.checkValueAgainstSchema(
          ...
          PARAMETER_SOURCE.REQUEST,
          components,
          _extend({}, options, { shortValidationErrors: true }),
          schemaCache,
          jsonSchemaDialect,
          callback
        );
      ...
      let urlencodedBodySchema = _get(schemaPath, [
          'requestBody',
          'content',
          URLENCODED,
          'schema'
      ...
      urlencodedBodySchema = deref.resolveRefs(
      ...
      _forEach(
        _get(urlencodedBodySchema, 'properties'),
        (propSchema, propName) => {
          let resolvedProp = {
              name: propName,
              schema: propSchema,
              in: 'query', // serialization follows same behaviour as query params
              description: _get(propSchema, 'description') || ''
            },
            encodingValue = _get(schemaPath, [
              'requestBody',
              'content',
              URLENCODED,
              'encoding',
          ...
          if (isObject(encodingValue)) {
            _has(encodingValue, 'style') &&
              (resolvedProp.style = encodingValue.style);
            _has(encodingValue, 'explode') &&
              (resolvedProp.explode = encodingValue.explode);
          }

          if (_includes(_get(urlencodedBodySchema, 'required'), propName)) {
            resolvedProp.required = true;
          }

          pSerialisationInfo = this.getParamSerialisationInfo(
            resolvedProp,
            PARAMETER_SOURCE.REQUEST,
            components,
            schemaCache
          );
          isPropSeparable = _includes(
            ['form', 'deepObject'],
            pSerialisationInfo.style
          );
          ...
          if (
             ...
             */
            if (
              !_includes(['array', 'object'], _get(propSchema, 'items.type'))
            ) {
              // add schema of corresponding items instead array
              resolvedSchemaParams.push(
                _assign({}, resolvedProp, {
                  schema: _get(propSchema, 'items'),
                  isResolvedParam: true
                })
              );
            }
            ...
            if (pSerialisationInfo.style === 'deepObject') {
              resolvedSchemaParams = _concat(
                resolvedSchemaParams,
                this.extractChildParamSchema(propSchema, propName, {
              ...
              // add schema of all properties instead entire object
              _forEach(_get(propSchema, 'properties', {}), (value, key) => {
                resolvedSchemaParams.push({
                  name: key,
                  schema: value,
                  isResolvedParam: true,
      ...
      return async.map(
        ...
        (uParam, cb) => {
          let mismatches = [],
            index = _findIndex(requestBody.urlencoded, uParam),
            resolvedParamValue = uParam.value;

          const schemaParam = _find(resolvedSchemaParams, (param) => {
            return param.name === uParam.key;
          });
          ...
          if (!schemaParam) {
            ...
            if (options.showMissingInSchemaErrors) {
              mismatches.push({
                property: mismatchProperty,
                transactionJsonPath:
                  transactionPathPrefix + `.urlencoded[${index}]`,
                schemaJsonPath: null,
                reasonCode: 'MISSING_IN_SCHEMA',
        ...
        (err, res) => {
          let mismatches = [],
            ...
            getPropNameFromSchemPath = (schemaPath) => {
              ...
              return _last(regex.exec(schemaPath));
            };

          // update actual value and suggested value from JSON to serialized strings
          _forEach(_flatten(res), (mismatchObj) => {
            if (!isEmpty(mismatchObj)) {
              let propertyName = getPropNameFromSchemPath(
                  mismatchObj.schemaJsonPath
                ),
                schemaParam = _find(resolvedSchemaParams, (param) => {
              ...
              if (schemaParam) {
                ...
                serializedParamValue = _get(
                  this.convertParamsWithStyle(
                    schemaParam,
                    _get(mismatchObj, 'suggestedFix.suggestedValue'),
                    PARAMETER_SOURCE.REQUEST,
                    components,
                    schemaCache,
                    options
                ...
                _set(
                  mismatchObj,
                  'suggestedFix.actualValue',
                  schemaParam.actualValue
                );
                _set(
                  mismatchObj,
                  'suggestedFix.suggestedValue',
                  serializedParamValue
                );
              }
            }
          ...
          _each(resolvedSchemaParams, (uParam) => {
            // report mismatches only for required properties
            if (
              !_find(requestBody.urlencoded, (param) => {
                return param.key === uParam.name;
              }) &&
              uParam.required
            ) {
              mismatchObj = {
  ...
  checkResponseBody: function (
    ...
    schemaContent = _get(schemaResponse, [
      'content',
      jsonContentType,
      'schema'
    ]);
    ...
    async.map(
      ...
      (response, responseCallback) => {
        let thisResponseCode = response.code,
          thisSchemaResponse = _get(schemaPath, [
            'responses',
            thisResponseCode
          ]),
        ...
        if (!thisSchemaResponse) {
          ...
          thisSchemaResponse = _get(schemaPath, ['responses', 'default']);
          responsePathPrefix = 'default';
        }

        // resolve $ref in response object if present
        if (!isEmpty(_get(thisSchemaResponse, '$ref'))) {
          thisSchemaResponse = this.getRefObject(
            thisSchemaResponse.$ref,
            components,
            options
          );
        }
        ...
        _forEach(_get(thisSchemaResponse, 'headers'), (header) => {
          if (_has(header, '$ref')) {
            _assign(
              header,
              this.getRefObject(header.$ref, components, options)
            );
            _unset(header, '$ref');
          }
        });
          ...
          async.parallel(
            ...
            (err, result) => {
              return responseCallback(null, {
                id: response.id,
                matched:
                  result.body.length === 0 && result.headers.length === 0,
  ...
  getPostmanUrlSchemaMatchScore: function (postmanPath, schemaPath, options) {
    var postmanPathArr = _reject(postmanPath.split('/'), (segment) => {
        return segment === '';
      }),
      schemaPathArr = _reject(schemaPath.split('/'), (segment) => {
        return segment === '';
      }),
    ...
    _each(postmanPathSuffixes, (pps) => {
      let suffixMatchResult = this.getPostmanUrlSuffixSchemaScore(
        pps,
        schemaPathArr,
        options
      );
  ...
  getPostmanUrlSuffixSchemaScore: function (pmSuffix, schemaPath, options) {
    ...
    if (
      options.strictRequestMatching &&
      pmSuffix.length !== schemaPath.length
    ) {
      return {
        match: false,
        score: null,
    ...
    for (let i = 0; i < minLength; i++) {
      let schemaFixedParts = this.getFixedPartsFromPathSegment(
          schemaPath[sMax - i],
          'schema'
        ),
        collectionFixedParts = this.getFixedPartsFromPathSegment(
          pmSuffix[pMax - i],
          'collection'
        );

      if (
        isEqual(schemaFixedParts, collectionFixedParts) || // exact fixed parts match
        ...
        else if (isEqual(schemaFixedParts, collectionFixedParts)) {
          fixedMatchedSegments++;
        }
        ...
        if (isSchemaSegmentPathVar(schemaPath[sMax - i])) {
          variables.push({
            key: schemaPath[sMax - i].substring(
              1,
              schemaPath[sMax - i].length - 1
            ),
    ...
    if (!mismatchFound) {
      return {
        ...
        fixedMatchedSegments,
        variableMatchedSegments,
        pathVars: _reverse(variables) // keep index in order of left to right
      };
    }
    return {
      match: false,
  ...
  getSuggestedValue: function (fakedValue, actualValue, ajvValidationErrorObj) {
    ...
    switch (ajvValidationErrorObj.keyword) {
      ...
      case 'minProperties':
        suggestedValue = _assign(
          {},
          targetActualValue,
          _pick(
            targetFakedValue,
            _difference(_keys(targetFakedValue), _keys(targetActualValue))
          )
        );
        break;

      case 'maxProperties':
        suggestedValue = _pick(
          targetActualValue,
          _intersection(_keys(targetActualValue), _keys(targetFakedValue))
        );
        break;
      ...
      case 'required':
        suggestedValue = _assign(
          {},
          targetActualValue,
          _pick(targetFakedValue, ajvValidationErrorObj.params.missingProperty)
        );
        break;

      case 'minItems':
        suggestedValue = _concat(
          targetActualValue,
          _slice(targetFakedValue, targetActualValue.length)
        );
        break;

      case 'maxItems':
        suggestedValue = _slice(
          targetActualValue,
          0,
          ajvValidationErrorObj.params.limit
        );
        break;

      case 'uniqueItems':
        tempSuggestedValue = _cloneDeep(targetActualValue);
        tempSuggestedValue[ajvValidationErrorObj.params.j] =
          _last(targetFakedValue);
   ...
   */
  getMissingSchemaEndpoints: function (
    schema,
    matchedEndpoints,
    components,
    options,
    ...
    _forEach(schemaPaths, (schemaPathObj, schemaPath) => {
      _forEach(_keys(schemaPathObj), (pathKey) => {
        schemaJsonPath = `$.paths[${schemaPath}].${_toLower(pathKey)}`;
        if (
          METHODS.includes(pathKey) &&
          !matchedEndpoints.includes(schemaJsonPath)
        ) {
          let mismatchObj = {
            property: 'ENDPOINT',
            ...
            reason: `The endpoint "${_toUpper(
              pathKey
            )} ${schemaPath}" is missing in collection`,
            endpoint: _toUpper(pathKey) + ' ' + schemaPath
          };

          if (options.suggestAvailableFixes) {
            let operationItem = _get(schemaPathObj, pathKey) || {},
              convertedRequest,
            ...
            // storing common path/collection vars from the server object at the path item level
            if (!isEmpty(_get(schemaPathObj, 'servers'))) {
            ...
            request = {
              ...
              method: pathKey,
              path:
                schemaPath[0] === '/' ? schemaPath.substring(1) : schemaPath,
              properties: operationItem,
              type: 'item',
              servers: isEmpty(_get(schemaPathObj, 'servers'))
            };

            // convert request to collection item and store collection variables
            convertedRequest = this.convertRequestToItem(
              schema,
              request,
              components,
              options,
