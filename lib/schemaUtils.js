...
module.exports = {
  ...
  assignParameterExamples: function (parameter) {
    ...
    if (example !== undefined) {
      ...
      !_.isUndefined(exampleToUse) &&
        _.set(parameter, 'schema.example', exampleToUse);
    }
  },
    ...
    _.forEach(securitySet, (security) => {
      ...
      if (!_.isObject(securityDef)) {
        ...
        if (_.toLower(securityDef.scheme) === 'basic') {
          ...
          _.toLower(securityDef.scheme) === 'oauth' ||
          _.toLower(securityDef.scheme) === 'oauth1'
        ) {
          helper = {
            type: 'oauth1',
            oauth1: [
        ...
        helper = {
          ...
          apikey: [
            {
              key: 'key',
              value: _.isString(securityDef.name)
                ? securityDef.name
                : '<API Key Name>',
            },
            ...
            {
              key: 'in',
              value: _.includes(['query', 'header'], securityDef.in)
                ? securityDef.in
                : 'header',
            },
          ],
  ...
  convertParamsWithStyle: function (
    ...
    switch (style) {
      case 'form':
        ...
        if (
          explode &&
          _.get(param, 'schema.type') === 'object' &&
          _.isEmpty(_.get(param, 'schema.properties'))
        ) {
          return pmParams;
        }
        break;
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      _.forOwn(bodyData, (value, key) => {
        if (_.get(contentObj[URLENCODED], 'schema.type') === 'object') {
          ...
          required = _.includes(
            _.get(contentObj[URLENCODED], ['schema', 'required']),
            key
          );
          enumValue = _.get(contentObj[URLENCODED], [
            'schema',
            'properties',
            key,
            'enum',
      ...
      _.forOwn(bodyData, (value, key) => {
        if (_.get(contentObj[FORM_DATA], 'schema.type') === 'object') {
          ...
          required = _.includes(
            _.get(contentObj[FORM_DATA], ['schema', 'required']),
            key
          );
          enumValue = _.get(contentObj[FORM_DATA], [
            'schema',
            'properties',
            key,
            'enum',
        ...
        originalParam = _.get(contentObj[FORM_DATA], [
          'schema',
          'properties',
          key,
        ]);
  ...
  convertRequestToItem: function (
    ...
    var reqName,
      ...
      localServers = fromWebhooks
        ? ''
        : _.get(operationItem, 'properties.servers'),
      exampleRequestBody,
      sanitizeResult,
    ...
    sanitizeResult.collectionVars.forEach((element) => {
      if (!variableStore[element.name]) {
        ...
        variableStore[element.name] = _.assign(convertedPathVar, {
          key: element.name,
          type: 'collection',
        });
      }
    ...
    if (
      ...
      item.request.url.variables.members = _.map(
        item.request.url.variables.members,
        (m) => {
    ...
    _.forEach(reqParams.header, (header) => {
      if (
        ...
        !_.includes(IMPLICIT_HEADERS, _.toLower(_.get(header, 'name')))
      ) {
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        if (options.includeAuthInfoInExample) {
          ...
          originalRequestQueryParams = _.concat(
            originalRequestQueryParams,
            authQueryParams
          );
          originalRequestHeaders = _.concat(
  ...
  findMatchingRequestFromSchema: function (method, url, schema, options) {
    ...
    _.forOwn(schemaPathItems, (pathItemObject, path) => {
      ...
      pathItemObject.parameters = _.reduce(
    ...
    filteredPathItemsArray = _.orderBy(
      filteredPathItemsArray,
    ...
    _.each(filteredPathItemsArray, (fp) => {
      ...
      // filter empty parameters
      matchedPath.parameters = _.reduce(
  ...
  isPmVariable: function (value) {
    ...
    return (
      _.isString(value) && _.startsWith(value, '{{') && _.endsWith(value, '}}')
    );
  },
  ...
  getPathValue: function (sourceValue, dataPath, fallback) {
    return dataPath === ''
      ? sourceValue
      : _.get(sourceValue, dataPath, fallback);
  },
  ...
  getSecurityParams: function (components, location) {
    ...
    _.forEach(securityDefs, (securityDef) => {
      ...
      if (
        _.get(securityDef, 'type') === 'apiKey' &&
        _.get(securityDef, 'in') === location
      ) {
  ...
  getParamSerialisationInfo: function (
    ...
    switch (param.in) {
      case 'path':
        style = _.includes(['matrix', 'label', 'simple'], param.style)
          ? param.style
          : 'simple';
        break;
      case 'query':
    ...
    explode = _.isBoolean(param.explode)
      ? param.explode
      : _.includes(['form', 'deepObject'], style);
      ...
      _.forEach(allProps, (element, index) => {
        ...
        if (paramSchema.type === 'object') {
          ...
          constructedValue.push(
            convertToDataType(_.get(keyValArray, '[1]', element))
          );
        }
      });
    } else {
    ...
    if (compositeSchema) {
      ...
      async.map(
        ...
        (elementSchema, cb) => {
          setTimeout(() => {
            this.checkValueAgainstSchema(
              ...
              }[${_.findIndex(compositeSchema, elementSchema)}]`,
              elementSchema,
              parameterSourceOption,
              components,
              options,
        ...
        (err, results) => {
          ...
          sortedResults = _.sortBy(results, (res) => {
            return res.length;
          });
    ...
    // When processing a reference, schema.type could also be undefined
      ...
      if (isKnownType(schema)) {
        ...
        if (!isCorrectType) {
          ...
          if (
            _.includes(['QUERYPARAM', 'PATHVARIABLE', 'HEADER'], property) &&
    ...
    async.map(
      ...
      (err, res) => {
        ...
        _.each(schemaPathVariables, (pathVar) => {
          if (
            !_.find(determinedPathVariables, (param) => {
  ...
  checkMetadata(
    ...
    actualReqDesc = _.isObject(_.get(transaction, 'request.description'))
      ? _.get(transaction, 'request.description.content')
      : _.get(transaction, 'request.description');
     ...
     */
    if (
      (!_.isEmpty(actualReqDesc) || !_.isEmpty(expectedReqDesc)) &&
  ...
  checkQueryParams(
    ...
    let parsedUrl = require('url').parse(requestUrl),
      schemaParams = _.filter(schemaPath.parameters, (param) => {
    ...
    securityParams = _.map(
      this.getSecurityParams(_.get(components, 'components'), 'query'),
      'name'
    );
    ...
    _.forEach(schemaParams, (param) => {
      ...
      if (isPropSeparable && paramSchema.type === 'array' && explode) {
         ...
         */
        if (
          !_.includes(['array', 'object'], _.get(paramSchema, 'items.type'))
        ) {
        ...
        if (style === 'deepObject') {
          resolvedSchemaParams = _.concat(
            resolvedSchemaParams,
            this.extractChildParamSchema(paramSchema, param.name, {
              required: _.get(param, 'required'),
              pathPrefix,
    ...
    return async.map(
      ...
      (pQuery, cb) => {
        ...
        const schemaParam = _.find(resolvedSchemaParams, (param) => {
      ...
      (err, res) => {
        ...
        _.each(
          _.filter(resolvedSchemaParams, (q) => {
            return q.required;
          }),
          (qp) => {
            if (
              !_.find(requestQueryParams, (param) => {
  ...
  checkContentTypeHeader: function (
    ...
    if (!_.isEmpty(contentHeader) && _.isEmpty(mediaTypes)) {
      ...
      if (
        ...
        _.toLower(contentHeaderMediaType) !== TEXT_PLAIN
      ) {
        mismatches.push({
          property: mismatchProperty,
          transactionJsonPath:
  ...
  checkRequestHeaders: function (
    ...
    let schemaHeaders = _.filter(schemaPath.parameters, (param) => {
      ...
      securityHeaders = _.map(
        this.getSecurityParams(_.get(components, 'components'), 'header'),
        'name'
      ),
      ...
      reqHeaders = _.filter(headers, (header) => {
        ...
        return (
          !_.includes(IMPLICIT_HEADERS, _.toLower(_.get(header, 'key'))) &&
    ...
    return async.map(
      ...
      (pHeader, cb) => {
        ...
        const schemaHeader = _.find(schemaHeaders, (header) => {
      ...
      (err, res) => {
        ...
        if (reqBody) {
          ...
          contentHeaderMismatches = this.checkContentTypeHeader(
            ...
            _.get(reqBody, 'content'),
            mismatchProperty,
            options
          );
        }
        _.each(
          _.filter(schemaHeaders, (h) => {
            // exclude non-required and implicit header from further validation
            return (
              h.required && !_.includes(IMPLICIT_HEADERS, _.toLower(h.name))
            );
          }),
          (header) => {
            if (
              !_.find(reqHeaders, (param) => {
        ...
        return callback(
          null,
          _.concat(contentHeaderMismatches, _.flatten(res), mismatches)
        );
      }
    );
  },
      ...
      (err, res) => {
        let mismatches = [],
          ...
          contentHeaderMismatches = this.checkContentTypeHeader(
            ...
            _.get(schemaResponse, 'content'),
            mismatchProperty,
            options
          );

        _.each(
          _.filter(schemaHeaders, (h, hName) => {
            ...
            // exclude non-required and implicit header from further validation
            return (
              h.required && !_.includes(IMPLICIT_HEADERS, _.toLower(hName))
            );
          }),
          (header) => {
            if (
              !_.find(resHeaders, (param) => {
                return param.key === header.name;
              })
            ) {
              // assign parameter example(s) as schema examples;
  ...
  checkRequestBody: function (
    ...
    jsonContentType = this.getJsonContentType(
      _.get(schemaPath, 'requestBody.content', {})
    );
    jsonSchemaBody = _.get(schemaPath, [
      'requestBody',
      'content',
      jsonContentType,
      'schema',
    ...
    if (requestBody && requestBody.mode === 'raw' && jsonSchemaBody) {
      ...
      let urlencodedBodySchema = _.get(schemaPath, [
          'requestBody',
          'content',
          URLENCODED,
          'schema',
      ...
      _.forEach(
        _.get(urlencodedBodySchema, 'properties'),
        (propSchema, propName) => {
          let resolvedProp = {
              name: propName,
              schema: propSchema,
            ...
            encodingValue = _.get(schemaPath, [
              'requestBody',
              'content',
              URLENCODED,
              'encoding',
          ...
          if (_.isObject(encodingValue)) {
            _.has(encodingValue, 'style') &&
              (resolvedProp.style = encodingValue.style);
            _.has(encodingValue, 'explode') &&
      ...
      return async.map(
        ...
        (err, res) => {
          ...
          _.forEach(_.flatten(res), (mismatchObj) => {
            if (!_.isEmpty(mismatchObj)) {
              let propertyName = getPropNameFromSchemPath(
                ...
                schemaParam = _.find(resolvedSchemaParams, (param) => {
              ...
              if (schemaParam) {
                ...
                serializedParamValue = _.get(
                  this.convertParamsWithStyle(
                    schemaParam,
                    _.get(mismatchObj, 'suggestedFix.suggestedValue'),
                    PARAMETER_SOURCE.REQUEST,
                    components,
                    schemaCache,
                    options
          ...
          _.each(resolvedSchemaParams, (uParam) => {
            ...
            if (
              !_.find(requestBody.urlencoded, (param) => {
  ...
  checkResponseBody: function (
    ...
    schemaContent = _.get(schemaResponse, [
      'content',
      jsonContentType,
      'schema',
    ]);
    ...
    async.map(
      ...
      (response, responseCallback) => {
        let thisResponseCode = response.code,
          thisSchemaResponse = _.get(schemaPath, [
            'responses',
            thisResponseCode,
          ]),
  ...
  getSuggestedValue: function (fakedValue, actualValue, ajvValidationErrorObj) {
    ...
    switch (ajvValidationErrorObj.keyword) {
      ...
      case 'minProperties':
        suggestedValue = _.assign(
          {},
          targetActualValue,
          _.pick(
            targetFakedValue,
            _.difference(_.keys(targetFakedValue), _.keys(targetActualValue))
          )
        );
        break;

      case 'maxProperties':
        suggestedValue = _.pick(
          targetActualValue,
          _.intersection(_.keys(targetActualValue), _.keys(targetFakedValue))
        );
        break;
      ...
      case 'minItems':
        suggestedValue = _.concat(
          targetActualValue,
          _.slice(targetFakedValue, targetActualValue.length)
        );
        break;

      case 'maxItems':
        suggestedValue = _.slice(
          targetActualValue,
          0,
