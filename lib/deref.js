const cloneDeep = require('lodash/cloneDeep'),
  ...
  _matches = require('lodash/matches'),
...
module.exports = {
  ...
  resolveRefs: function (
    ...
    if (schema.anyOf) {
      ...
      return {
        anyOf: _map(schema.anyOf, (schemaElement) => {
          return this.resolveRefs(
            schemaElement,
            parameterSourceOption,
            components,
    ...
    if (schema.oneOf) {
      ...
      return {
        oneOf: _map(schema.oneOf, (schemaElement) => {
          return this.resolveRefs(
            schemaElement,
            parameterSourceOption,
            components,
    ...
    if (schema.$ref && isFunction(schema.$ref.split)) {
      ...
      if (splitRef.length < 4) {
        ...
        return {
          value: 'reference ' + schema.$ref + ' not found in the OpenAPI spec'
        };
      }
      ...
      if (outerProperties) {
        resolvedSchema = concreteUtils.addOuterPropsToRefSchemaIfIsSupported(
          resolvedSchema,
          outerProperties
        );
      }
      if (resolvedSchema) {
        let refResolvedSchema = this.resolveRefs(
          resolvedSchema,
          parameterSourceOption,
          components,
          schemaResolutionCache,
    ...
    if (
      ...
      if (_has(schema, 'properties') || _has(schema, 'additionalProperties')) {
        // shallow cloning schema object except properties object
        let tempSchema = _omit(schema, ['properties', 'additionalProperties']);

        if (_has(schema, 'additionalProperties')) {
          ...
          if (isBoolean(schema.additionalProperties)) {
            ...
            tempSchema.additionalProperties = this.resolveRefs(
        ...
        !isEmpty(schema.properties) && (tempSchema.properties = {});
        for (prop in schema.properties) {
          if (schema.properties.hasOwnProperty(prop)) {
            ...
            if (
              property.readOnly &&
              parameterSourceOption === PARAMETER_SOURCE.REQUEST
            ) {
              // remove property from required as it'll not be present in resolved schema
              if (_includes(tempSchema.required, prop)) {
                _remove(tempSchema.required, _matches(prop));
              }
              continue;
            }
            else if (
              property.writeOnly &&
              parameterSourceOption === PARAMETER_SOURCE.RESPONSE
            ) {
              // remove property from required as it'll not be present in resolved schema
              if (_includes(tempSchema.required, prop)) {
                _remove(tempSchema.required, _matches(prop));
            ...
            /* eslint-enable */
            tempSchema.properties[prop] = this.resolveRefs(
              property,
              parameterSourceOption,
              components,
              schemaResolutionCache,
      ...
      concreteUtils.compareTypes(schema.type, SCHEMA_TYPES.array) &&
      schema.items
    ) {
      /*
      ...
      */
      if (resolveFor === 'CONVERSION') {
        ...
        if (
          !_has(schema, 'minItems') &&
          _has(schema, 'maxItems') &&
          schema.maxItems >= 2
        ) {
          schema.minItems = 2;
        }
        ...
        if (_has(schema, 'minItems') && schema.minItems > 0) {
          schema.maxItems = schema.minItems;
        }

        // If no maxItems is defined than override with default (2)
        !_has(schema, 'maxItems') && (schema.maxItems = 2);
      }
      ...
      let tempSchema = _omit(schema, 'items');
      tempSchema.items = this.resolveRefs(
        schema.items,
        parameterSourceOption,
        components,
        schemaResolutionCache,
      ...
      if (
        !_includes(SUPPORTED_FORMATS, schema.format) ||
        (schema.pattern && schema.format)
      ) {
        return _omit(schema, 'format');
      }
    }
