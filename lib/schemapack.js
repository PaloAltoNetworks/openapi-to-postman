...
class SchemaPack {
  constructor(input, options = {}) {
    ...
    this.computedOptions.indentCharacter =
      indentCharacter === 'tab' ? '\t' : '  ';
  ...
  validate() {
    ...
    if (input.type === 'string' || input.type === 'json') {
      ...
      this.validationResult = {
        result: false,
        reason:
          'Input data not validated, please call mergeAndValidate() for input.type \'folder\''
      };
  ...
  getMetaData(cb) {
    ...
    // if the schema is validated, return the meta data as required.
      ...
      return cb(null, {
        result: true,
        name: _get(this.openapi, 'info.title', COLLECTION_NAME),
        output: [
          {
            type: 'collection',
            name: _get(this.openapi, 'info.title', COLLECTION_NAME)
  ...
  mergeAndValidate(cb) {
    ...
    try {
      rootFiles = parse.getRootFiles(
        input,
        concreteUtils.inputValidation,
    ...
    if (rootFiles.length) {
      parse
        .mergeFiles(rootFiles[0], OasResolverOptions, files)
        .then((spec) => {
          this.input = {
            type: 'json',
            data: spec
  ...
  convert(callback) {
    ...
    if (!this.validated) {
      return callback(
        new OpenApiErr(
          'The schema must be validated before attempting conversion'
        )
      );
    }
    ...
    Object.assign(
      specComponentsAndUtils,
      concreteUtils.getRequiredData(this.openapi)
    );

    // create and sanitize basic spec
    openapi = this.openapi;
    openapi.servers = isEmpty(openapi.servers) ?
      [{ url: '/' }] :
      openapi.servers;
    openapi.securityDefs = _get(openapi, 'components.securitySchemes', {});
    openapi.baseUrl = _get(openapi, 'servers.0.url', '{{baseURL}}');

    // TODO: Multiple server variables need to be saved as environments
    openapi.baseUrlVariables = _get(openapi, 'servers.0.variables');
    ...
    generatedStore.collection = new sdk.Collection({
      info: {
        name: isEmpty(_get(openapi, 'info.title')) ?
          COLLECTION_NAME :
          _get(openapi, 'info.title')
      }
    });
    ...
    schemaUtils
      .convertToPmCollectionVariables(
    ...
    generatedStore.collection.describe(
      schemaUtils.getCollectionDescription(openapi)
    );
  ...
  validateTransaction(transactions, callback) {
    ...
    // create and sanitize basic spec
    schema.servers = isEmpty(schema.servers) ? [{ url: '/' }] : schema.servers;
    schema.securityDefs = _get(schema, 'components.securitySchemes', {});
    schema.baseUrl = _get(schema, 'servers.0.url', '{{baseURL}}');
    schema.baseUrlVariables = _get(schema, 'servers.0.variables');
    ...
    try {
      ...
      return callback(
        new OpenApiErr('Invalid syntax provided for requestList', e)
      );
    }
    ...
    return setTimeout(() => {
      async.map(
        transactions,
        (transaction, requestCallback) => {
          if (!transaction.id || !transaction.request) {
            return requestCallback(
              new Error(
                'All transactions must have `id` and `request` properties.'
              )
            );
          }
          ...
          if (typeof requestUrl === 'object') {
            ...
            _forEach(requestUrl.variable, (pathVar) => {
              if (
                isNil(pathVar.value) ||
                (typeof pathVar.value === 'string' &&
                  _trim(pathVar.value).length === 0)
              ) {
          ...
          return setTimeout(() => {
            ...
            return async.map(
              ...
              (matchedPath, pathsCallback) => {
                ...
                _forEach(matchedPath.pathVariables, (pathVar) => {
                  let mappedPathVar = _find(
                    _get(transaction, 'request.url.variable', []),
                    (transactionPathVar) => {
                      return transactionPathVar.key === pathVar.key;
                    }
                  );
                  pathVar.value = _get(mappedPathVar, 'value', pathVar.value);
                  // set _varMatched flag which represents if variable was found in transaction or not
                  pathVar._varMatched = !isEmpty(mappedPathVar);
                });

                // resolve $ref in all parameter objects if present
                _forEach(_get(matchedPath, 'path.parameters'), (param) => {
                  if (param.hasOwnProperty('$ref')) {
                    _assign(
                      param,
                      schemaUtils.getRefObject(
                        param.$ref,
                        componentsAndPaths,
                        options
                      )
                ...
                async.parallel(
                  {
                    ...
                    path: function (cb) {
                      schemaUtils.checkPathVariables(
                        matchedPath.pathVariables,
                    ...
                    headers: function (cb) {
                      schemaUtils.checkRequestHeaders(
                  ...
                  (err, result) => {
                    ...
                    _each(result.responses, (response) => {
                      if (_get(response, 'mismatches', []).length > 0) {
        ...
        (err, result) => {
          ...
          retVal = {
            requests: keyBy(result, 'requestId'),
            missingEndpoints: schemaUtils.getMissingSchemaEndpoints(
              schema,
              matchedEndpoints,
              componentsAndPaths,
              options,
