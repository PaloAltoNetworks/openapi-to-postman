...
const parse = require('../parse.js'),
  ...
  schemaTypeToJsValidator = {
    ...
    boolean: (d) => _isBoolean(d) || d === 'true' || d === 'false',
...
function checkValueOnlyTypes(value, types) {
  return Array.isArray(types) ?
    isTypeValueArrayCheck(value, types) :
    compareType(value, types);
}
...
module.exports = {
  ...
  handleExclusiveMaximum: function (schema, max) {
    max = schema.hasOwnProperty('maximum') ? schema.maximum : max;
  ...
  handleExclusiveMinimum: function (schema, min) {
    min = schema.hasOwnProperty('minimum') ? schema.minimum : min;
  ...
  formatSchemaPathFromAJVErrorToConvertToDataPath: function (schemaPath) {
    return removeWordFromLastPosition(
      removeWordFromLastPosition(
        removeSharpAndSlashFromFirstPosition(schemaPath),
        'type'
      ),
      'format'
    );
