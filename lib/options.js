  ...
  args.forEach((argument) => {
    if (typeof argument === 'object' && Object.keys(argument).length > 0) {
      criteria =
        ...
        SUPPORTED_VERSIONS.includes(argument.version)
  ...
  getOptions: function (mode = 'document', criteria = {}) {
    ...
    let optsArray = [
      ...
      },
      {
        ...
        default: true,
        description:
          'Importing will collapse all folders that have only one child element and lack ' +
      ...
      },
      {
        ...
        default: true,
        description:
          'Optimizes conversion for large specification, disabling this option might affect' +
      ...
      },
      {
        ...
        default: 10,
        description:
          'Number of nesting limit till which schema resolution will happen. Increasing this limit may' +
      ...
      },
      {
        ...
        default: false,
        description:
          'MISSING_IN_SCHEMA indicates that an extra parameter was included in the request. For most ' +
      ...
      },
      {
        ...
        default: false,
        description:
          'Determines whether to show detailed mismatch information for application/json content ' +
      ...
      },
      {
        ...
        default: false,
        description:
          'Whether requests should be strictly matched with schema operations. Setting to true will not ' +
      ...
      },
      {
        ...
        default: false,
        description:
          'Whether to allow matching path variables that are available as part of URL itself ' +
    ...
    if (isObject(criteria)) {
      typeof criteria.external === 'boolean' &&
        (optsArray = _filter(optsArray, { external: criteria.external }));
      if (isArray(criteria.usage)) {
        let tempOptsArray = [];

        _forEach(criteria.usage, (usageCriteria) => {
          tempOptsArray = _concat(
            tempOptsArray,
            _filter(optsArray, (option) => {
              return _includes(option.usage, usageCriteria);
            })
          );
        });
