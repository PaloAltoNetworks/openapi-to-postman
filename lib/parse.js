...
module.exports = {
  asJson: function (spec) {
    try {
      ...
      throw new SyntaxError(
        `Specification is not a valid JSON. ${jsonException}`
      );
    }
  },
  ...
  asYaml: function (spec) {
    try {
      ...
      throw new SyntaxError(
        `Specification is not a valid YAML. ${yamlException}`
      );
    }
  },
  ...
  mergeFiles: function (source, options = {}, files = {}) {
    ...
    if (!isEmpty(files)) {
      ...
      return new Promise((resolve, reject) => {
        return this.resolveContent(unresolved, options, files).then(
          (result) => {
