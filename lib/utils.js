const _toLower = require('lodash/toLower'),
  ...
  _isSafeInteger = require('lodash/isSafeInteger'),
    ...
    for (let id in defaultOptions) {
      if (defaultOptions.hasOwnProperty(id)) {
        ...
        if (userOptions[id] === undefined) {
          ...
          if (defaultOptions[id].type === 'enum' && _isString(retVal[id])) {
            retVal[id] = _toLower(defaultOptions[id].default);
          }
          continue;
        }
        ...
        switch (defaultOptions[id].type) {
          ...
          case 'enum':
            ...
            if (
              defaultOptions[id].availableOptions.includes(userOptions[id]) ||
              (_isString(userOptions[id]) &&
                _map(defaultOptions[id].availableOptions, _toLower).includes(
                  _toLower(userOptions[id])
                ))
            ) {
              retVal[id] = userOptions[id];
            } else {
            ...
            // ignore case-sensitivity for string options
            _isString(retVal[id]) && (retVal[id] = _toLower(retVal[id]));
          ...
          case 'array':
            ...
            break;
          case 'integer':
            if (_isSafeInteger(userOptions[id])) {
              retVal[id] = userOptions[id];
            } else {
              retVal[id] = defaultOptions[id].default;
            }
